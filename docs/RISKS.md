...
| `AS-04` | App Store | Guideline 2.5.2 — current Prototo App architecture (`expo-dev-client` + Metro bundle download via cloudflared tunnel) downloads & executes code at runtime. `docs/prototo-app-store-plan.md` argues this is a textbook 2.5.2 violation and that SDUI rebuild is required for approval. | `open` | `docs/prototo-app-store-plan.md`; `/Users/sherizan/.claude/plans/drifting-weaving-hammock.md` | Intentional empirical probe: submit current arch, use Apple's actual response (rejection / approval / info-request) as ground truth before committing to the 4–6 week SDUI pivot. Decision tree in the plan file. Resolves when Apple responds. |
