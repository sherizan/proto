...
| `AS-04` | App Store | Guideline 2.5.2 — Prototo App architecture (`expo-dev-client` + Metro bundle download via cloudflared tunnel) downloads & executes code at runtime. `docs/prototo-app-store-plan.md` argues this is a textbook 2.5.2 violation and that SDUI rebuild is required for approval. | `pending-resubmit` | `docs/prototo-app-store-plan.md`; `/Users/sherizan/.claude/plans/drifting-weaving-hammock.md` | First submission attempt: Build 0.1.2 (#4) on 2026-05-25 built with `developmentClient: false` (EAS production-profile default). Withdrawn from review before Apple started — the bundle-load code path was compiled out, so it would not have actually probed 2.5.2. Rebuilt as Build 0.1.3 with `developmentClient: true` added to eas.json's production profile. This binary genuinely embeds the dev-client URL handler, so it can load remote bundles via Metro/tunnel. Local TestFlight smoke test required before clicking Submit for Review. Resolves on Apple's verdict. |
