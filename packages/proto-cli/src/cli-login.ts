...
<style>body{font-family:-apple-system,system-ui,sans-serif;display:grid;place-items:center;height:100vh;margin:0;background:#0b0b0c;color:#fff}div{text-align:center}</style>
...
/** Start a one-shot loopback server on an OS-assigned port. */
export function startLoopbackServer(): Promise<LoopbackServer> {
  return new Promise((resolve, reject) => {
    ...
    const server = createServer((req: IncomingMessage, res: ServerResponse) => {
      ...
      if (onHit && req.url) onHit(req.url);
    });
