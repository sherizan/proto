...
describe('runShare — cloud-streaming flow', () => {
  ...
  it('does not walk the screens: Publish app is the app only (#25)', async () => {
    const createShare = vi.fn(makeDeps({}).createShare);
    const log = vi.fn();
    await runShare({ cliOverride: undefined }, makeDeps({ createShare, log }));
    expect(createShare.mock.calls[0]?.[0]).not.toHaveProperty('screenCount');
    expect(log.mock.calls.flat().some((m) => String(m).includes('Capturing'))).toBe(false);
    expect(log).toHaveBeenCalledWith('Uploading your prototype…');
  });
